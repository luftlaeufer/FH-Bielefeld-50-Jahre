const sketch5 = (p) => {
    ...
    let maxHeight = 0.0; // bezieht sich auf die maximale pixelHöhe wobei height ein auschnitt aus 0 - maxheight ist

    let ausmasse; //Ausmaße ist die Skalierung der anzuzeigenden Daten 
    let position; //Position verschiebt das Koordinatensystem

    let farben1 = new p5.Table(); // farben1 ist ne Tabelle mit den Fraben bis in die Gegenwart 2 für die Zukunft
    let farben2 = new p5.Table(); // 0 ist immer BG, 1 die tabelle und der Rest der Gaph

    let worter1 = new p5.Table();
    let worter2 = new p5.Table();
    let worter3 = new p5.Table();

    let werte = new p5.Table();
    ...
    p.preload = function () {
        ...
        // Farben wurden mehrfach kopiert
        farben1 = p.loadTable("../assets/data/05_farben1.csv", 'csv', "header"); // Vergangenheit
        farben2 = p.loadTable("../assets/data/05_farben2.csv", 'csv', "header"); // Zukunft
        ...
        Graph1 = new Graph(werte, worter1, worter2, worter3); // der Graph wird compiled
    ...
    // alles, was nicht Graph ist. Farben, Namen, Labels, predictions please
    class Static {
    ...
    class Graph {
        ...
        constructor(tabelle, worter1, worter2, worter3) {
            this.RWC1 = new RandomWordCombiner(worter1, worter2, worter3);
            ...
            this.rechteSchranke = this.linkeSchranke + this.zeitlange;;
            ...
            //noiseSeedBase ist nen Array mit verschieden Werten von -100 bis 100 um immer neue Noise werte zu generieren aber diese nihct zu verlieren
            //this.noiseSeedBase = new Array((this.tabelle.getColumnCount() - 1) * 100);
            this.noiseSeedBase = new Array((this.tabelle.getColumnCount()) * 100);
            
            for (let i = 0; i<this.noiseSeedBase.length-1; i++) {
                this.noiseSeedBase[i] = p.random(-100, 100);
              }

            this.mdcb = 0; // missing data colour
        }
        ...
        anzeigen = (farben) => {
          ...
          //p.strokeWeight(this.supplements.getNum(this.supplements.getRowCount(), "strokeWeight"));
          p.strokeWeight(2);
          ...
          for (let i = 0; i<this.tabelle.getColumnCount()-1 - 2; i++) {
            if (i<farben.getRowCount()-1 -2) {
              this.ausschnittZeichnen(this.linkeSchranke, this.rechteSchranke, i, p.color(farben.getNum(i +2, "r"), farben.getNum(i +2, "g"), farben.getNum(i +2, "b")));
            } else if (i>farben.getRowCount()-1 -2 && i<(farben.getRowCount()-1 -2)*2) {
              let p = i - (farben.getRowCount()-1);
              this.ausschnittZeichnen(this.linkeSchranke, this.rechteSchranke, i, p.color(farben.getNum(p +2, "r"), farben.getNum(p +2, "g"), farben.getNum(p +2, "b")));
            } else {
              this.ausschnittZeichnen(this.linkeSchranke, this.rechteSchranke, i, p.color(farben.getNum(farben.getRowCount()-1, "r"), farben.getNum(farben.getRowCount()-1, "g"), farben.getNum(farben.getRowCount()-1, "b")));
            }
          }
        }
...
/*                 p.stroke(
                ...
                if (this.tabelle.getNum(i, spalte)<0 ) {
                  p.line(
        ...
        graphLabelsZeichnen (c, farben) {
          for (let i = 0; i < this.tabelle.getRowCount(); i++) {
            ...
            p.text(1971 + (i)/2, -p.width+3.4*p.width/40, p.map((i), 0, this.maximaleLange, 0, maxHeight)-p.height/53,57);
        ...
        unLoadFuture = () => {
          ...
          //um kein OutofBoundsError zu kriegen wird erst geprüft
          if (this.linkeSchranke -1 > 1) {
      
            //wenn man in der Zukunft ist werden Zeilen gelöscht nachdem man sie rückgängig gemacht hat
            if (this.rechteSchranke >= this.originalLange -1) {
              this.tabelle.removeRow(this.tabelle.getRowCount()-1);
              this.supplements.removeRow(this.supplements.getRowCount()-1);
            }
        ...
        getRechteSchranke = () => {
          return this.rechteSchranke;
        }
      
        getZeitlange = () => {
          return this.zeitlange;
        }
        ...
        // Cleared die Labels, wo Daten 0 sind
        getNamenVonNichtNull = (minus) => {
          let lange=0;
          for (let i = 0; i < this.tabelle.getColumnCount()-1; i++) {
            if (this.tabelle.getNum(this.rechteSchranke - minus, i) != 0) {
              lange ++;
            }
          }
      
          let temp = new Array(this.tabelle.getColumnCount()-1);
          let temp2 = new Array(lange+1);
      
          let ii=0;
          for (let i = 0; i < this.tabelle.getColumnCount()-1; i++) {
            temp[i] = this.tabelle.getColumnTitle(i);
            if (ii<=lange) {
              if (this.tabelle.getNum(this.rechteSchranke - minus, i) != 0) {
                temp2[ii] = temp [i];
                ii ++;
        ...
        getLangeVonNichtNull = (minus) => {
          let lange=0;
          for (let i = 0; i < this.tabelle.getColumnCount()-1; i++) {
            if (tabelle.getNum(this.rechteSchranke - minus, i) != 0) {
              lange ++;
            }
          }
          return lange;
        ...
        fillSupplements = () => {
          ...
          for (let i = 0; i < this.tabelle.getRowCount(); i++) {
            let maxColWert = 0;
            for (let ii = 0; ii < this.tabelle.getColumnCount(); ii++) {
              if (maxColWert<this.tabelle.getNum(i, ii)) {
                maxColWert = this.tabelle.getNum(i, ii);
              }
            }
            //println(maxColWert);
            ...
            // also nicht {12427814} sondern {12447888}
             if ((i == 0) || (maxColWert > this.supplements.getNum(i-1, "maxWerte")) ) {
              this.supplements.setNum(i, "maxWerte", maxColWert);
            } else {
              this.supplements.setNum(i, "maxWerte", this.supplements.getNum(i-1, "maxWerte"));
            } 
          }
      
          //strokeWeight
          for (let i = 0; i < this.tabelle.getRowCount(); i++) {
            this.supplements.setNum(i, "strokeWeight", 4);
          }
      
          //opacity
          for (let i = 0; i < this.tabelle.getRowCount(); i++) {
            this.supplements.setNum(i, "opacity", 255);
          }
        }
        ...
        randomize = () => {
          ...
          if (r>80 && r<96) {
            this.tabelle.setNum(
              this.tabelle.getRowCount()-1, 
              p.int(p.random(0, this.tabelle.getColumnCount()-1)), 
              p.random(0, this.supplements.getNum(this.supplements.getRowCount()-1, "maxWerte")));
          } 
          ...
          if (r>70 && r<80) {
            this.tabelle.setNum(
              this.tabelle.getRowCount() - 1, 
              p.int(random(0, this.tabelle.getColumnCount()-1)), 2);
          }
      
          //Die linien werden Fett und halb transparent
          if (r>54 && r<60) {
            if (p.supplements.getNum(p.supplements.getRowCount()-2, "strokeWeight")*12 < 120) {
              p.supplements.setNum(p.supplements.getRowCount()-1, "strokeWeight", (p.supplements.getNum(p.supplements.getRowCount()-2, "strokeWeight")*12));
              p.supplements.setNum(p.supplements.getRowCount()-1, "opacity", (p.supplements.getNum(p.supplements.getRowCount()-2, "opacity")/1.5));
            } else {
              p.supplements.setNum(p.supplements.getRowCount()-1, "strokeWeight", 100);
              p.supplements.setNum(p.supplements.getRowCount()-1, "opacity", (p.supplements.getNum(p.supplements.getRowCount()-2, "opacity")*p.random(0.1, 1.2)));
            }
          }
          ...
          if (r>0 && r<0.5) {
            for (let i = 0; i< this.tabelle.getColumnCount()-1; i++) {
              this.tabelle.setNum(this.tabelle.getRowCount()-1, i, 0);
            }
      
            this.tabelle.addColumn("Studiengang");
            for (let i =0; i< this.tabelle.getRowCount()-1; i++) {
              this.tabelle.setNum(i, "Studiengang", 0);
            }
            this.tabelle.setNum(this.tabelle.getRowCount()-1, "Studiengang", p.random(0, 2000));
          }
          ...
          // Scale wird komisch
          if (r>20 && r<25) {
            this.supplements.setNum(this.supplements.getRowCount()-1, "maxWerte", (this.supplements.getNum(supplements.getRowCount()-2, "maxWerte")*p.random(0.25, 2)));
          }
        }
        ...
        addNewCol = () => {
          if (this.getLangeVonNichtNull(5)<19) { 
            let r = this.RWC1.getRandomWord();
            this.tabelle.addColumn(r);
            for (let i =0; i< this.tabelle.getRowCount()-1; i++) {
              this.tabelle.setNum(i, r, 0);
            }
            this.tabelle.setNum(this.tabelle.getRowCount()-1, r, p.random(0, 2000));
          }
        }
    }
